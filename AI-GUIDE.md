# EDU KIT — AI向け統合ガイド

## この文書の役割

小学校向けWeb教材を作成・改修するAIは、最初にこの文書を読みます。

edu-kitは、5つの共通資産を一つに複製した場所ではありません。学習目標に必要な資産を選び、各リポジトリの正本へ案内し、最後に教材全体の接続と回帰を確認するための統合ハブです。

5つの共通基盤は次の役割で分担します。

- `edu-components`：動作・ロジック・状態・保存・進行
- `edu-effects`：UI・CSS・視覚フィードバック・モーション
- `sounds-recipe-`：Web Audio APIによる効果音
- `edu-assets`：バッジ・エレメント・コレクションなどの画像資産
- `navi-character-`：ナビキャラ・学習シーン・ファンタジー・モンスター画像

5つはすべて必須ではありません。学習目標に必要なものだけを使います。詳細をedu-kit側へコピーして管理せず、各リポジトリの実装・AI-GUIDE・公開カタログを正本として確認してください。

## 絶対ルール

1. **作業前に現状を確認する。** 既存教材では、ソースのファイル構成、問題データ、正答、判定、進行、保存、画像リンク、公開画面を先に確認する。
2. **保護対象と変更対象を分ける。** ユーザーが触らないと指定した領域、既存の学習内容、問題数、保存形式を勝手に変更しない。
3. READMEの説明だけでAPI、CSSクラス、音ID、画像URLを推測しない。
4. 参照先の実装ファイルと公開カタログで、利用する名前・引数・イベントdetail・パスの実在を確認する。
5. **5基盤を全部入れることを目的にしない。** 学習目標に必要な基盤だけを選ぶ。
5a. **音を使う教材では、音の実装を任意工程にしない。** 正解・不正解・操作・達成などに音によるフィードバックを採用する場合は、`sounds-recipe-` を必ず調査し、実在する音IDを選定・接続・再生確認まで行う。
6. 既存部品と同じ処理を教材側で再実装しない。教材固有の問題データ、画面文言、正答、達成条件は教材リポジトリ側に置く。
7. **状態と判定の正本はedu-components側に置く。** effects、sound、assets、navi-characterは、判定結果や状態変化を受けて表示・演出する側として接続する。
8. Managerの内部へDOM、教材CSS、音、画像を混在させない。
8a. **音声再生は実装しただけで完了としない。** `sounds.js` と `SOUND_GUIDE.md` で実在する `soundList` のIDを確認し、`AudioContext` のユーザー操作開始・resumeを含む再生経路を実装する。ブラウザ上で実際に音が鳴ること、ミュート時に鳴らないこと、音が失敗しても学習本体が止まらないことを確認する。状態とイベントを受けた教材側が表示を接続する。
9. 新規教材はHTML / CSS / Vanilla JavaScript、GitHub Pages、localStorageを基本とし、npm、ビルド、APIキー、外部DBを前提にしない。
10. タッチとマウスに対応し、必要な場面ではキーボードにも対応する。長押し・ドラッグだけを必須操作にしない。
11. **入口から学習内容を選べるようにする。** 複数のモードや単元がある場合は、開始前に何をするかを明確に選べるようにし、選択中の状態を見た目で分かるようにする。
12. **タブレット横向きの1画面を基準にする。** 問題、観察対象、主要操作、現在地を優先し、巨大なタイル・絵・ヘッダーで学習領域を圧迫しない。スクロールは必要な補足領域だけにする。
13. **つまずいても進めるようにする。** 問題の性質に応じて、ヒント、解説、答えを見る、やり直す、次へ進むなどの脱出口を用意する。答えを見た場合は正答・マスター達成として偽って記録しない。
14. **シミュレーターは現象を主役にする。** 「条件を変える → その場で現象が変わる → 短く結果を確かめる → もう一度試す」の循環を優先し、長い記録入力や説明文を操作より前に置かない。
15. 保存は教材固有namespaceで管理する。新規教材は原則としてStorageManagerを使い、既存教材では保存キー・スキーマ・過去データを調査してから変更する。リロード後も進捗・獲得状態・設定が保持されるか確認する。
16. 報酬は学習行動と結びつける。未獲得のバッジを最初から色付きで表示せず、獲得条件と状態を分けて扱う。獲得演出で学習を長時間止めない。
17. Web表示では軽量WebPを優先する。原本PNG/JPGは高解像度、印刷、制作・再編集、または軽量版がない場合だけ使う。軽量版の実在を確認せずに拡張子だけを置き換えない。
18. 絵文字、キャラクター、演出だけで学習内容を表さない。問題文・式・選択肢・観察結果を読みやすく表示し、装飾が文字や式より大きくなりすぎないようにする。
19. アニメーションは意味・位置・速度を確認する。白いちらつき、不要な画面全体の点滅、レイアウトシフト、対象物と説明のずれを起こさない。reduced motionでは簡略化する。
20. 画像・音・演出の読み込み失敗で学習本体を止めない。補助要素は失敗時に非表示または代替表示にできるようにする。
21. **存在しない共通APIを新しく発明しない。** 既存の実装に合わない場合は教材固有の接続を書くか、共通基盤側へ追加する提案と実装範囲を分ける。
22. **URL・パス・ファイル名・イベント名・音ID・CSSクラス名を推測しない。** 実在確認できないものは使用しない。
23. 完了と報告の前に、基本フロー、保存、端末表示、コンソールエラー、参照URL、5基盤との接続状態を確認する。

## AIが編集前に整理すること

作業を始める前に、次の5点を短く書き出します。

| 項目 | 書く内容 |
| --- | --- |
| 学習目標 | 何ができるようになればよいか |
| 保護対象 | 変更しない問題、正答、判定、保存、画像、画面 |
| 変更対象 | 今回直す画面・機能・ファイル |
| 共通資産 | 使用する基盤、使用しない基盤、実在確認先 |
| 完了確認 | 開始、回答、つまずき、結果、リロード、端末幅、接続確認 |

これを先に整理できない場合は、コードを変更せず、現状確認を続けます。

## 参照順序

1. ユーザーの依頼から学年・教科・単元・学習目標・変更範囲・保護対象を抜き出す。
2. 既存教材なら、リポジトリのファイル、問題データ、判定、保存、画像参照、公開サイトを確認する。
3. DESIGN-PRINCIPLES.mdで学習体験とUIの優先順位を確認する。
4. 該当するskills/*/SKILL.mdを選ぶ。
5. この文書の目的別レシピで候補を絞る。
6. **使う基盤ごとに正本を確認する。** edu-componentsは実装とAI-GUIDE、edu-effectsはCSS本体とカタログ、sounds-recipe-はsounds.jsとSOUND_GUIDE、edu-assetsはBadge Labと実ファイル、navi-character-はcatalog.jsonとAI-GUIDEを確認する。
7. 学習骨格を先に実装し、補助資産を後から接続する。
8. 接続後にイベント、保存、画像、音、CSSの実在と失敗時の継続を確認する。
9. **音を採用した教材では、音IDの実在だけでなく実再生まで確認する。** 特にiPad・Safari等のユーザー操作前後のAudioContext状態、連打時の重なり、音量、ミュート、ページ遷移後の再生を確認する。
10. 音が入るべき教材なのに音が鳴らない場合は「接続済み」「完成」と報告せず、原因を特定して修正する。
9. final-reviewに沿って回帰確認する。

## 5基盤の役割と正本

| 基盤 | 担当 | 正本として確認するもの | 公開カタログ |
| --- | --- | --- | --- |
| edu-components | 動作・ロジック・状態管理 | [index.js](https://github.com/TT-sensei/edu-components/blob/main/index.js) / [AI-GUIDE](https://github.com/TT-sensei/edu-components/blob/main/AI-GUIDE.md) | [Catalog](https://tt-sensei.github.io/edu-components/) |
| edu-effects | UI・CSS・視覚演出 | [css/](https://github.com/TT-sensei/edu-effects/tree/main/css) / [AI-GUIDE](https://github.com/TT-sensei/edu-effects/blob/main/AI-GUIDE.md) | [Catalog](https://tt-sensei.github.io/edu-effects/) |
| sounds-recipe- | Web Audio APIの効果音 | [sounds.js](https://github.com/TT-sensei/sounds-recipe-/blob/main/sounds.js) / [SOUND_GUIDE](https://github.com/TT-sensei/sounds-recipe-/blob/main/SOUND_GUIDE.md) | [Catalog](https://tt-sensei.github.io/sounds-recipe-/) |
| edu-assets | バッジ・エレメント・コレクション画像 | [assets/](https://github.com/TT-sensei/edu-assets/tree/main/assets) / Badge Lab | [Badge Lab](https://tt-sensei.github.io/edu-assets/) |
| navi-character- | ナビキャラ・学習シーン・ファンタジー画像 | [catalog.json](https://github.com/TT-sensei/navi-character-/blob/main/catalog.json) / [AI-GUIDE](https://github.com/TT-sensei/navi-character-/blob/main/AI-GUIDE.md) | [Character Library](https://tt-sensei.github.io/navi-character-/) |

機械可読の入口はedu-kit.jsonです。

## 接続アーキテクチャ

```text
学習目標
  ↓
edu-kit
  ↓
edu-components（状態・判定・進行の正本）
  ↓
EDU_EVENTS
  ├─ edu-effects（視覚フィードバック）
  ├─ sounds-recipe-（音）
  ├─ edu-assets（バッジ・画像）
  └─ navi-character-（キャラクター・シーン）
  ↓
教材画面
  ↓
保存・回帰確認
```

例えば正解時は、教材側の判定を二重に作らず、edu-components側の状態変化を基準にします。その結果を受けて、必要なら正解演出、correct音、バッジ、ナビキャラのcelebratingなどを接続します。

演出・音・キャラクターは学習内容の代わりではありません。学習本体がなくても演出だけが成立する設計にはしないでください。

## 既存教材の改修

既存教材を改修する場合は、次の順に確認します。

1. 現在のソースと公開サイトを見て、実際に使われている入口・単元・問題・保存キーを特定する。
2. 問題データ、正答、判定、問題数、学習順序、保存スキーマを「保護対象」として記録する。
3. ユーザーが変更してよい範囲だけを変更対象にする。
4. 画像URLとCSS・JSの読み込み先を実ファイルで確認する。
5. UIを直す場合も、問題と進行を先に通してから見た目を整える。
6. 変更後に、旧データを持つ状態・新規状態・リロード状態を確認する。
7. 画像や演出がなくても、開始・回答・進行・結果・ホーム復帰ができることを確認する。

「全面的に作り直す」と依頼された場合でも、問題内容と保存を無条件に捨てる意味にはしません。ゼロベースにする範囲を、画面構成・UI・演出・ロジック・データごとに分けて判断します。

## シミュレーターの設計

理科などのシミュレーターは、説明を読ませるページではなく、条件を触って現象を確かめる場として設計します。

- 変えられる条件を少数に絞り、現在値を常に見えるようにする。
- 条件変更がその場で図・動き・数値・色などの現象へ反映されるようにする。
- 何が変わったかを短い結果表示で確かめられるようにする。
- リセット、初期化、別条件への再試行を用意する。
- 操作パネルが観察対象を隠さない。
- 「シミュレーション」と「問題を解く」を分ける場合、開始画面と選択状態を明確に分ける。
- 長文の実験記録や入力を必須にせず、必要なら短い確認問題・ラベル・結果だけを置く。
- 図形・光線・回路・月・植物などは、説明用の線やラベルが対象とずれないか確認する。
- 1画面に収まらない場合は、操作・観察・結果の順に優先し、補足説明を後ろへ置く。

## イベント連携

イベント名とdetailの正本はedu-componentsの実装で確認します。ここにある名前を固定APIとして扱わず、実装更新時には必ず再確認してください。

| イベント | 主な用途 | 接続例 |
| --- | --- | --- |
| edu:correct | 正解 | 正解演出 + correct |
| edu:wrong | 不正解 | 不正解演出 + wrong / softFail |
| edu:screenchange | 画面切替 | ページ遷移演出 |
| edu:combo | コンボ更新 | コンボ表示 + combo3 / combo5 / combo10 |
| edu:timerwarning | 残り時間警告 | タイマー警告 + warning |
| edu:timeup | 時間切れ | 終了表示 + timeUpSoft |
| edu:newrecord | 新記録 | 記録表示 + 記録用サウンド |
| edu:progress | 進捗更新 | 進捗バー更新 |
| edu:levelchange | レベル変更 | レベルアップ演出 + levelup |
| edu:unlock | 項目解放 | 解放演出 + unlock |
| edu:achievement | 新しい達成 | 達成演出 + mission |
| edu:badge | 新しいバッジ獲得 | event.detail.badge.image + 獲得演出 + badge |

接続時は、イベントを発火させる側と受け取る側の両方を確認します。イベント名だけ合っていてdetailの形が違う状態を「接続済み」とみなさないでください。

## 目的別レシピ

### 基本クイズ

- logic：ScreenManager、QuestionPool、問題形式、ScoreManager
- style：問題UI、選択肢、正誤フィードバック
- answer support：必要に応じてヒント・解説・答えを見る・次へ進む
- sound：`correct`、`wrong` または `softFail` を `sounds-recipe-` の実在する `soundList` から確認して接続する。音を採用した場合は、ユーザー操作後のAudioContext再開と実機再生まで確認する。
- asset：報酬が学習を支える場合だけ

### 60秒チャレンジ

- logic：QuestionPool、Challenge60、ScoreManager、ComboManager
- style：タイマー、スコア、コンボ、ランクカード
- sound：correct、コンボ音、warning、timeUpSoft
- asset：新記録や達成に報酬を付ける場合だけ
- check：時間切れ、再挑戦、タイマー停止、二重回答を確認する

### 条件を試すシミュレーター

- logic：教材固有の状態、必要ならStorageManagerとProgressManager
- style：条件パネル、観察領域、短い結果表示、リセット
- cycle：条件を変える → 現象が変わる → 短く確かめる → 再試行
- avoid：長文記録、巨大な説明カード、操作より前に出る演出

### キャラクターで案内する教材

- character：navi-character-のカタログから実在画像を選ぶ
- logic：既存の問題・正誤ロジック
- style：キャラクターを主役にしすぎず、問題と操作を最優先に配置
- example：開始=waving、正解=correct、ヒント=hint、再挑戦=retry、完了=complete
- check：集合画像1枚、通常フィードバック約52px、画像失敗時の継続

### バッジ付き教材

- logic：StorageManager、AchievementManager、BadgeManager
- style：獲得済み・未獲得・次の条件が区別できる表示
- sound：badge、必要ならrareBadge、mission
- asset：Badge Labで確認した軽量WebPの実在URL

## edu-assetsとnavi-character-の画像

- フォルダ名からURLを組み立てて推測しない。
- Badge Lab、Character Library、catalog.json、実ファイルで実在パスを確認する。
- Web表示はassets/web/以下の軽量WebPを先に探し、存在しない場合だけ原本を使う。
- キャラクターは問題文、選択肢、答え、判定の代わりにしない。
- 未獲得のバッジ・コレクションは、獲得済みと区別できる状態で表示する。
- 獲得条件が別々の項目を、一つのシリーズ獲得として最初からまとめて表示しない。

## 完成前チェック

- 学習目標と問題形式またはシミュレーションの操作が一致している。
- 既存教材の保護対象を意図せず変更していない。
- 開始画面で、何をするか・どこを押すか・選択中の状態が分かる。
- タブレット横向きで問題・観察・主要操作が1画面に収まる。
- 使用するimport名、メソッド、イベント名とdetailが実装に存在する。
- 読み込むCSSファイルと利用クラスが実在する。
- 音IDがsoundListに存在する。
- **音を採用した場合、実際のブラウザ操作で音が再生される。** `AudioContext` の状態、ユーザー操作によるresume、ミュート、連打、音量を確認する。
- **音が鳴らない状態を実装完了としない。**
- バッジ・キャラクター画像がカタログまたは実ファイルに存在する。
- Web版画像がある場合、原本ではなく軽量WebPを使っている。
- 問題文・式・選択肢が絵文字や装飾より読みやすい。
- シミュレーターは条件変更に対して現象が即時に変わり、リセットして再試行できる。
- 答えを見る・ヒント・次へなど、必要な脱出口があり、答えを見た記録を正答と混同していない。
- 保存namespaceが教材固有で、リロード後も進捗・報酬・設定が保持される。
- 色だけで正誤を伝えていない。
- 白いちらつき、レイアウトシフト、対象のずれがない。
- prefers-reduced-motionでも意味が伝わる。
- タッチ、マウス、必要に応じてキーボードで操作できる。
- 画像・音・演出が失敗しても学習本体が止まらない。
- 5基盤を使う場合、各基盤の実装・カタログ・イベント接続が実在する。
- ホーム→開始→出題/観察→正解→不正解/つまずき→次問/再試行→結果→ホーム→再スタートを通せる。

## AIへの短い依頼テンプレート

~~~text
制作前に https://github.com/TT-sensei/edu-kit を起点として確認してください。
DESIGN-PRINCIPLES.md、AI-GUIDE.md、該当する skills/*/SKILL.md を読んでください。
既存教材では、編集前に問題データ・正答・判定・保存・画像リンクを確認し、保護対象と変更対象を分けてください。
READMEだけで判断せず、使う共通基盤の実装・AI-GUIDE・公開カタログでAPI、イベント、CSSクラス、音ID、画像URLの実在を確認してください。
edu-componentsを状態・判定の正本として扱い、必要な場合だけedu-effects、sounds-recipe-、edu-assets、navi-character-をイベント経由で接続してください。
タブレット横向きの1画面を基準に、学習入口・問題/観察・主要操作を優先してください。
子どもがつまずいても、ヒント・解説・答えを見る・再挑戦などから次の行動へ進めるようにしてください。
実在確認できないAPI、CSSクラス、音ID、画像URL、イベントdetailは推測で実装しないでください。
最後に保存、リロード、端末幅、コンソールエラー、5基盤の接続を確認してください。
音を採用した教材では、sounds-recipe-の正本（sounds.js / SOUND_GUIDE.md）で音IDを実在確認し、ユーザー操作後にAudioContextがresumeされ、実際に音が鳴ることまで確認してください。音が鳴らない場合は完成扱いにしないでください。
~~~
